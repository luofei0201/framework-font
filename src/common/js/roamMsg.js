 ...
 * Created by Wang  。
 */
// 对于POST请求,需要使用x-www-form-urlencoded的形式传参，因为云API侧是从$_POST中取出请求参数的。
...
var baseHost = 'console.tim.qq.com'
...
var QcloudApi = function(defaults) {
  this.defaults = assign({
    path: '/v4/group_open_http_svc/group_msg_get_simple',
    method: 'POST',
    protocol: 'https',
    baseHost: baseHost,
...
QcloudApi.prototype.generateQueryString = function(data, opts) {
    ...
    var param = assign({
      ...
      Nonce: Math.round(Math.random() * 65535) // 随机正整数，与 Timestamp 联合起来， 用于防止重放攻击。
